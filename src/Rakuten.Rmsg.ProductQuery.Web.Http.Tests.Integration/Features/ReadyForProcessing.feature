...
Scenario: Flagging a product query as ready for processing persists the correct information to the database
	...
	Then the status of the product query in the database is Submitted
...
Scenario: Flagging a product query as ready for processing creates a message on the queue
	...
	And the message for the new product query has the correct product query identifier
	And the message for the new product query has the correct culture
	And the message for the new product query has the correct enclosure link
...
Scenario: Flagging a product query as ready for processing with an invalid GUID returns the correct response
	...
	And the HTTP problem detail for the ready for processing request is The product query identifier '{id}' in the request URI is invalid. It must be a GUID.
...
Scenario: Flagging a product query as ready for processing with an invalid culture returns the correct response
	...
	And the HTTP problem is of type http://problems.rakuten.com/invalid-request-parameter
	And the HTTP problem title is An invalid request parameter was supplied.
	And the HTTP problem detail for the ready for processing request is The culture '{culture}' in the request URI is invalid. It must be a valid language tag (as per BCP 47).
...
Scenario: Flagging a product query as ready for processing with an identifier that does not exist returns the correct response
	...
	And the HTTP problem detail for the ready for processing request is Failed to find a product query with identifier '{id}'.
...
Scenario: Flagging a product query as ready for processing and supplying an invalid status returns the correct response
	...
	And the HTTP problem detail for the ready for processing request is You attempted to update the status of the query at '/product-query/{id}/culture/{culture}' to '{status}', which is an invalid status.  The only valid status to which this query can be set is 'submitted'.
...
Scenario: Flagging a product query as ready for processing when it has already been flagged as such returns the correct response
	...
	And the HTTP problem detail for the ready for processing request is You attempted to flag the query at '/product-query/{id}/culture/{culture}' as ready for processing but has already been flagged as such.
	And the HTTP problem contains a link of relation type http://rels.rakuten.com/product-query
	And the HTTP problem link of relation type http://rels.rakuten.com/product-query has an href that points to the product query